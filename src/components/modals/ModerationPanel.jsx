  ...
  return (
    <div className="animate-fade-in pb-10">
      <div className="mb-6 flex items-start justify-between gap-4 md:mb-8">
        <div>
          <h2 className="text-2xl font-bold tracking-tight text-[var(--text-main)] font-display">Moderation Queue</h2>
          <p className="mt-1 text-sm text-gray-500">Review user reports and record a supported action.</p>
        </div>
      ...
      {reports.length === 0 ? (
        <div className="premium-section rounded-2xl p-10 text-center">
          <CheckCircle2 className="mx-auto mb-3 text-emerald-400" size={34} />
          <h3 className="font-bold text-[var(--text-main)]">Queue is clear</h3>
          <p className="mt-1 text-sm text-gray-500">There are no open or escalated reports.</p>
        </div>
      ) : (
        <div className="grid gap-4 lg:grid-cols-[minmax(0,0.9fr)_minmax(0,1.2fr)]">
          <div className="premium-section max-h-[55vh] space-y-2 overflow-y-auto rounded-2xl p-2 custom-scrollbar">
            {reports.map(report => (
              <button key={report.id} type="button" onClick={() => setSelectedId(report.id)} className={`w-full rounded-xl border p-3 text-left transition-colors cursor-pointer ${selectedId === report.id ? 'border-indigo-500/60 bg-indigo-500/10' : 'border-transparent hover:bg-[var(--bg-element)]'}`}>
                <div className="flex items-center justify-between gap-3">
                  <span className="text-xs font-bold uppercase tracking-wider text-indigo-300">{report.target_type}</span>
                  <span className={`rounded-full px-2 py-0.5 text-[10px] font-bold uppercase ${report.status === 'reviewing' ? 'bg-amber-500/15 text-amber-300' : 'bg-red-500/15 text-red-300'}`}>{report.status}</span>
                </div>
                <p className="mt-2 line-clamp-2 text-sm font-semibold text-[var(--text-main)]">{reasonLabels.get(report.reason) || report.reason}</p>
                <p className="mt-1 text-[11px] text-gray-500">{new Date(report.created_at).toLocaleString()}</p>
              </button>
            ))}
          </div>

          {selectedReport && (
            <form onSubmit={handleModerate} className="premium-section rounded-2xl p-5">
              <div className="mb-4 flex items-center gap-2 text-sm font-bold text-[var(--text-main)]"><ShieldAlert size={18} className="text-red-400" />Report evidence</div>
              <div className="rounded-xl border border-[var(--border-subtle)] bg-[var(--bg-base)]/60 p-4">
                <p className="whitespace-pre-wrap break-words text-sm text-gray-300">{describeTarget(selectedReport)}</p>
              </div>
              {selectedReport.details && <div className="mt-3 rounded-xl bg-[var(--bg-element)] p-3 text-sm text-gray-400"><span className="font-bold text-gray-300">Reporter details: </span>{selectedReport.details}</div>}
              <div className="mt-3 flex items-center gap-1 text-[10px] text-gray-600"><ExternalLink size={12} />Target ID: {selectedReport.target_id}</div>

              <label className="mt-5 block">
                <span className="mb-2 block text-xs font-bold uppercase tracking-widest text-gray-400">Action</span>
                <select value={action} onChange={event => setAction(event.target.value)} className="premium-input h-11 w-full rounded-xl px-3 text-sm">
                  {availableActions.map(option => <option key={option.value} value={option.value}>{option.label}</option>)}
                </select>
              </label>
              <label className="mt-4 block">
                <span className="mb-2 block text-xs font-bold uppercase tracking-widest text-gray-400">Moderator note</span>
                <textarea required value={note} onChange={event => setNote(event.target.value.slice(0, 2000))} rows={4} className="premium-input w-full resize-none rounded-xl p-3 text-sm" placeholder="Record why this action is appropriate." />
              </label>
