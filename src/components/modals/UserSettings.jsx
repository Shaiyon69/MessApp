...
export default function UserSettingsModal({ session, initialTab = 'account', initialMobileMenu = true, onClose }) {
  const [loading, setLoading] = useState(false)
  const [activeTab, setActiveTab] = useState(initialTab)
  const [showMobileMenu, setShowMobileMenu] = useState(initialMobileMenu)
  ...
  useEffect(() => {
      async function getProfile() {
        const { data } = await supabase.from('profiles').select('avatar_url, banner_url, username, unique_tag, bio, pronouns, encrypted_private_key').eq('id', session.user.id).single()
        if (data) {
        ...
        <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 animate-fade-in pt-[env(safe-area-inset-top)] pb-[env(safe-area-inset-bottom)] pl-[env(safe-area-inset-left)] pr-[env(safe-area-inset-right)]">
          <div className="bg-[#15171a] w-full max-w-sm rounded-3xl border border-[#23252a] shadow-2xl p-6 text-center animate-slide-up md:animate-fade-in">
            ...
            <div className="flex flex-col gap-3">
              ...
              <button onClick={() => setShowLogoutConfirm(false)} className="w-full h-14 md:h-12 bg-white/5 hover:bg-white/10 text-gray-300 rounded-xl font-bold transition-all cursor-pointer border border-white/5 text-base md:text-sm">Cancel</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 animate-fade-in pt-[env(safe-area-inset-top)] pb-[env(safe-area-inset-bottom)] pl-[env(safe-area-inset-left)] pr-[env(safe-area-inset-right)]">
          <div className="bg-[#15171a] w-full max-w-sm rounded-3xl border border-red-500/50 shadow-2xl p-6 text-center animate-slide-up md:animate-fade-in">
            ...
            <div className="flex flex-col gap-3">
              ...
              <button onClick={() => setShowDeleteConfirm(false)} disabled={isDeleting} className="w-full h-14 md:h-12 bg-white/5 hover:bg-white/10 text-gray-300 rounded-xl font-bold transition-all cursor-pointer border border-white/5 text-base md:text-sm disabled:opacity-50">Cancel</button>
            </div>
          </div>
        </div>
      )}
      ...
      <div className="w-full h-full md:max-w-5xl md:h-[85vh] md:min-h-[600px] flex flex-col md:flex-row overflow-hidden md:rounded-2xl animate-slide-up shadow-2xl bg-[#0d0f12] md:border border-[#23252a]">
        ...
        <aside className={`${showMobileMenu ? 'flex' : 'hidden'} md:flex w-full md:w-64 bg-[#111214] md:bg-[#15171a] border-r border-[#23252a] flex-col shrink-0 z-20 h-full`}>
          <div className="flex md:hidden items-center justify-between w-full px-6 h-16 bg-[#0d0f12] border-b border-[#23252a] shrink-0 sticky top-0">
             ...
             <button onClick={onClose} className="p-2 bg-white/5 rounded-full text-gray-400 hover:text-white transition-colors cursor-pointer"><X size={24} /></button>
          </div>
          ...
          <div className="flex flex-col p-4 md:p-0 gap-4 md:gap-1 flex-1 overflow-y-auto">
            <div className="bg-[#1c1e22] md:bg-transparent rounded-2xl md:rounded-none border border-[#23252a] md:border-none overflow-hidden shadow-sm md:shadow-none">
              {TABS.map((tab, index) => (
                <div key={tab.id}>
                  <button onClick={() => { setActiveTab(tab.id); setShowMobileMenu(false); }} className={`w-full flex items-center justify-between md:justify-start gap-2 md:gap-3 px-5 md:px-3 h-16 md:h-10 font-medium text-base md:text-sm transition-all focus-visible:ring-2 focus-visible:ring-indigo-500 outline-none cursor-pointer ${activeTab === tab.id && !showMobileMenu ? 'md:bg-[#23252a] md:text-white md:shadow-sm' : 'text-gray-300 md:text-gray-400 hover:bg-white/5 hover:text-white bg-transparent'}`}>
                    <div className="flex items-center gap-3"><tab.icon size={20} className="md:w-[18px] md:h-[18px] text-gray-400" /> <span className="text-white md:text-inherit">{tab.label}</span></div>
        ...
        <main className={`${!showMobileMenu ? 'flex' : 'hidden'} md:flex flex-1 flex-col overflow-hidden bg-[#0d0f12] relative`}>
          ...
          <div className="flex-1 overflow-y-auto custom-scrollbar relative p-4 sm:p-8 md:p-14 pb-32 md:pb-14">
            <div className="max-w-2xl mx-auto h-full">
                    ...
                    <div className="p-5 sm:p-6 space-y-6">
                      ...
                      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                        ...
                        <div className="flex gap-2 w-full sm:w-auto">
                          {isEditingEmail ? (
                            <><button type="button" onClick={() => { setIsEditingEmail(false); setNewEmail(userEmail) }} className="bg-white/5 hover:bg-white/10 text-white px-4 h-14 md:h-10 rounded-xl md:rounded-lg font-medium transition-colors cursor-pointer flex-1 sm:flex-none text-base md:text-sm">Cancel</button><button type="button" onClick={handleUpdateEmail} disabled={loading} className="bg-indigo-500 hover:bg-indigo-600 text-white px-4 h-14 md:h-10 rounded-xl md:rounded-lg font-medium transition-colors cursor-pointer flex-1 sm:flex-none flex items-center justify-center gap-2 text-base md:text-sm">{loading ? <Loader2 size={18} className="animate-spin"/> : 'Save'}</button></>
                          ) : (
              ...
              {activeTab === 'privacy' && (
                <div className="animate-fade-in pb-10">
                  ...
                  <div className="space-y-4">
                    ...
                    <ToggleSwitch label="Allow direct messages from server members" description="This setting is applied when you join a new server." checked={allowDirectMessages} onChange={setAllowDirectMessages} />
                          ...
                          <div key={profile.id}>
                            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between p-4 hover:bg-white/5 transition-colors gap-4">
                              <div className="flex items-center gap-3">
                                ...
                                <div className="flex flex-col"><span className="text-white font-bold text-base md:text-sm">{profile.username}</span><span className="text-gray-500 text-sm md:text-xs font-mono">{profile.unique_tag}</span></div>
                              </div>
                    ...
                    <div className="bg-[#1c1e22] rounded-2xl ghost-border overflow-hidden">
                      {restrictedProfiles.length === 0 ? <div className="p-6 text-center text-gray-500 text-sm">You haven't restricted anyone.</div> : (
              ...
              {activeTab === 'security' && (
                <div className="animate-fade-in pb-10">
                  <h2 className="hidden md:block text-2xl font-bold tracking-tight text-white mb-6 md:mb-8 font-display">Secure Storage</h2>

                  <div className="bg-[#1c1e22] rounded-xl p-6 ghost-border shadow-sm">
                    <div className="flex items-center gap-3 mb-4">
                      ...
                      <div>
                        <h3 className="text-white font-bold text-lg">{hasSecureStorage ? 'Secure Storage Enabled' : 'Message Backup PIN'}</h3>
                        <p className="text-xs text-gray-400">{hasSecureStorage ? 'Your keys are safely backed up in the cloud.' : 'Encrypt your keys to the cloud'}</p>
                      </div>
                    </div>
                    
                    <p className="text-sm text-gray-300 mb-6 leading-relaxed">
                      {hasSecureStorage 
                        ? "Your End-to-End Encryption keys are backed up. If you want to change your PIN, enter a new 6-digit code below. This will re-encrypt your current keys." 
                        : "Create a 6-digit PIN to securely back up your End-to-End Encryption keys to the MessApp servers. When you log in on a new device, you will need this PIN to restore your chat history."}
                    </p>

                    <div className="flex flex-col md:flex-row gap-3">
                      <input id="pin-setup-input" type="password" maxLength="6" placeholder="••••••" className="bg-[#0d0f12] border border-[#23252a] rounded-xl px-4 py-3 text-white text-center tracking-[0.5em] font-mono text-xl md:w-48 outline-none focus:border-indigo-500 transition-colors" />
                      <button 
                        onClick={async () => {
                        ...
                        className={`flex-1 text-white px-6 py-3 rounded-xl font-bold transition-all cursor-pointer flex items-center justify-center gap-2 ${hasSecureStorage ? 'bg-[#23252a] hover:bg-gray-700' : 'bg-indigo-500 hover:bg-indigo-600 shadow-lg'}`}
                      >
                        <Shield size={18} /> {hasSecureStorage ? 'Update PIN' : 'Turn On Secure Storage'}
                      </button>
                    </div>
                  </div>
